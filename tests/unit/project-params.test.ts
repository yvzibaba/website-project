...
import {
  ...
  resolveProjectParamsBaseline,
} from "@app/kernel/server/project-params";
...
const SPEC_KEYS = PARAMETER_SPECS.map((s) => s.key);

describe("project-params · 结构与诚实不变式", () => {
  it("版本号合规", () => {
    expect(PARAMS_VERSION).toMatch(/^\d+\.\d+\.\d+$/);
  });

  it("PROJECT_PARAMS 已通过 ParameterSetSchema 校验（模块加载即 parse 成功）", () => {
    // 若集合有重复键 / min>max / derived 缺 dependsOn，import 时就 throw，走到这里即证合法。
    expect(PROJECT_PARAMS.length).toBe(PARAMETER_SPECS.length);
    expect(PROJECT_PARAMS.length).toBeGreaterThan(30);
  });
  ...
  it("五组参数均有覆盖（region/policy/project/technology/finance）", () => {
    const groups = new Set(PROJECT_PARAMS.map((s) => s.group));
  ...
  it("派生参数：键集合与 spec.derived 一致、且全部只读(editable=false)带 dependsOn", () => {
    const derivedSpecs = PROJECT_PARAMS.filter((s) => s.derived);
    expect(derivedSpecs.map((s) => s.key).sort()).toEqual([...DERIVED_KEYS].sort());
    for (const s of derivedSpecs) {
      expect(s.editable).toBe(false);
      expect(s.dependsOn && s.dependsOn.length).toBeGreaterThan(0);
      expect(typeof DERIVED[s.key]).toBe("function");
    }
  });

  it("每个派生 dependsOn 指向的键都真实存在于参数集（防止悬空依赖）", () => {
    const set = new Set(SPEC_KEYS);
    for (const s of PROJECT_PARAMS.filter((x) => x.derived)) {
      for (const d of s.dependsOn ?? []) expect(set.has(d), `${s.key} 依赖缺失 ${d}`).toBe(true);
    }
  });
});

describe("resolveProjectParamsBaseline · 基线解析与派生真值", () => {
  const base = resolveProjectParamsBaseline();
  ...
  it("确定性：两次基线解析深相等", () => {
    expect(resolveProjectParamsBaseline()).toEqual(base);
  });
});

describe("resolveProjectParams · §4 命脉：改上游参数 → 派生值重算（非只改页面数字）", () => {
  it("改桩数（8→10）→ 充电总功率重算 3600、并连带影响其它层不变", () => {
    const r = resolveProjectParams({ user: { values: { "project.chargerCount": 10 } } });
  ...
  it("地区层覆写电价 → origin=region、派生层照常重算", () => {
    const r = resolveProjectParams({ region: { values: { "region.elecPrice": 0.95 }, source: "示例省" } });
  ...
  it("储能功率置 0 → 储能时长除零无定义 → 诚实回落占位默认 2 + note（绝不裸抛/绝不算成 Infinity）", () => {
    const r = resolveProjectParams({ user: { values: { "project.storagePower": 0 } } });
  ...
  it("分层曝光：basic 档只含核心滑块（不含 finance 细则）", () => {
    const basic = filterByExposure(resolveProjectParamsBaseline(), "basic").map((p) => p.key).sort();
...
describe("project-params · V1.1 批次1.3：inactive 未启用标注（方案 §3.1d）", () => {
  ...
  it("未启用键集合 = 恰这 6 个（多标/漏标都红）", () => {
    const keys = PROJECT_PARAMS.filter((s) => s.inactive).map((s) => s.key).sort();
    expect(keys).toEqual(INACTIVE_SET);
  });

  it("每个未启用键都带用户可读的中文原因（诚实：收起必须解释为什么）", () => {
    for (const s of PROJECT_PARAMS.filter((x) => x.inactive)) {
  ...
  it("includeStorage **不标** inactive（批次 1.4 已真接线转正·MODEL 1.4.0 真杠杆，永远是活参数）", () => {
    const spec = PROJECT_PARAMS.find((s) => s.key === "project.includeStorage");
  ...
  it("活参数绝不被误标：已被内核消费的键全部在用", () => {
    ...
    for (const k of LIVE) {
      const s = PROJECT_PARAMS.find((x) => x.key === k);
  ...
  it("反向错配修正：region.demandCharge / project.demandKc exposure 升 advanced（B/C 对照组工作台可切）", () => {
    expect(PROJECT_PARAMS.find((s) => s.key === "region.demandCharge")!.exposure).toBe("advanced");
    expect(PROJECT_PARAMS.find((s) => s.key === "project.demandKc")!.exposure).toBe("advanced");
  });

  it("★行为反证：覆写任一 inactive 键 → NPV 逐字节不变（「未启用」= 对经济输出零影响，名副其实）", () => {
    const base = runProjectModel({});
    expect(base.ok).toBe(true);
    ...
    for (const [key, probe] of Object.entries(probes)) {
      const r = runProjectModel({ user: { values: { [key]: probe } } });
