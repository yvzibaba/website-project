 ...
 *   R5 的 `regions.ts` 把山西的电价/光照/补贴等作为**层级 `source` 自由文本**给了值，但整层共用一句
 ...
 *
 * 与参数目录的关系（§16）：只使用 `PROJECT_PARAMS` 里**已注册的键**，且**只给溯源元数据、不重复给数值**
 *   （数值仍只在 `regions.ts` 的 `values` 里，防双写漂移）。键合法性由 `tests/unit/region-facts.test.ts` 守。
 */
...
/**
 * 山西**地区层**逐值溯源（键须与 `regions.ts` 的 `SHANXI_PACK.region.values` 对齐）。
...
/** 一个地区的逐值溯源集合（供 `regions.ts` 在编排处 attach 到对应 ValueLayer）。 */
