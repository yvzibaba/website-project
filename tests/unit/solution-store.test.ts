...
/**
 * 单测 `persistSolutionDraft`（R8.2 落库编排）——**只测编排的分派与失败诚实**，不触库。
 *
...
// V1.1 P4：solution-source 新增了对 @/server/authz 的 STAFF_ROLES 引用（ownsSolutionSource 需要）。
...
import {
  persistSolutionDraft,
  SolutionPersistSchema,
  SOLUTION_STORE_VERSION,
} from "@app/kernel/server/solution-store";
...
describe("solution-store · persistSolutionDraft（R8.2 落库编排，mock solution-admin）", () => {
  ...
  it("needsProfessionalReview 缺省时编排层兜底为 true（§21 高风险须人工确认，宁保守）", async () => {
    ...
    void _drop;
    await persistSolutionDraft(rest, "human:tester");
  ...
  it("入参非法（caseId 非 cuid）→ status invalid + fieldErrors.caseId，createSolution 绝不被调用", async () => {
    const res = await persistSolutionDraft(baseInput({ caseId: "not-a-cuid" }), "human:tester");
  ...
  it("负价格被编排层 schema 拒（Decimal 禁负号）→ invalid.price，且不落 createSolution", async () => {
    const res = await persistSolutionDraft(baseInput({ price: "-1.00" }), "human:tester");
    expect(res.status).toBe("invalid");
  ...
  it("unknown 个别失败 → 同样记 warnings、unknownCount 只数成功；financials/unknowns 皆空则不建条目", async () => {
    ...
    const resA = await persistSolutionDraft(
    ...
    mockFin.mockClear();
    mockUnk.mockClear();
    const resB = await persistSolutionDraft(baseInput({ financials: [], unknowns: [] }), "human:tester");
