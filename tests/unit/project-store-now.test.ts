...
import { toEngineLayers, type StoredParamLayers } from "@app/kernel/server/project-store";
...
const shanxiStr = (now: string) => buildProjectLayers("shanxi", {}, new Date(now));

describe("project-store · toEngineLayers（R6.3 复活 now / 政策日期窗）", () => {
  ...
  it("★§6 实证：同一山西输入，now 落在 2024 政策窗口内 vs 之前 → NPV 必不同（现行政策确有生效）", () => {
    const active = runProjectModel(shanxiStr(NOW_ACTIVE));
    const before = runProjectModel(shanxiStr(NOW_BEFORE));
  ...
  it("★命脉可复算：山西此刻保存的输入，JSON 落库（全串）后回放重算与原值逐位相同", () => {
    ...
    const direct = runProjectModel(original);
    expect(direct.ok).toBe(true);
    const fromDb = JSON.parse(JSON.stringify(original)) as StoredParamLayers; // Date→串（正是回放入口）
    const replay = runProjectModel(toEngineLayers(fromDb));
  ...
  it("★混合护栏：now=Date 而政策窗=串（未复活）会把 2024 政策误判为现行；复活后回到正确值", () => {
    ...
    const correct = runProjectModel(shanxiStr(NOW_BEFORE));
    ...
    const naive = runProjectModel(mixed); // 未复活：Date<串 → NaN → 比较恒 false → 2024 政策被误当现行
    const revived = runProjectModel(toEngineLayers(mixed as StoredParamLayers)); // 复活 → Date 比 Date → 正确跳过
