 ...
 *   - `buildProfileLayers` 只是把 R5 的 `buildProjectLayers` 多垫一层画像预设，地区包 bounds 仍透传进 user 层，
 *     因此**复用同一套已测死的解析 / 经济 / 敏感性内核**，不新增参数、不新增计算、不碰数据库。
 *   - 报告的「企业个性化视角」节（见 `@/lib/decision-report`）只从**已算好的**指标卡里**挑选与排序**，
 ...
 *
 * 命名与键：只使用 `@/server/project-params` 里**已注册的键**（`project.*` / `finance.*` 等），不新增参数、
...
import { getRegionPack, buildProjectLayers } from "@app/kernel/server/regions";
...
export function profileCalcRef(): string {
  return `profiles@${PROFILES_VERSION}`;
}
...
const REGISTERED_KEYS = new Set(PROJECT_PARAMS.map((s) => s.key));

/** 报告/解释关注的指标卡 key（与 `@/lib/decision-view` 的 summaryCards 对齐）。 */
...
const GENERIC_PROFILE: EnterpriseProfile = {
  id: "generic",
  name: "通用（未设定画像）",
...
const FLEET_PROFILE: EnterpriseProfile = {
  id: "fleet",
  name: "物流 / 重卡车队运营商",
...
const OPERATOR_PROFILE: EnterpriseProfile = {
  id: "operator",
  name: "第三方充电运营商",
...
const PARK_PROFILE: EnterpriseProfile = {
  id: "park",
  name: "产业园区 / 工商业业主",
...
const TRANSIT_PROFILE: EnterpriseProfile = {
  id: "transit",
  name: "公交 / 市政集团",
...
const INVESTOR_PROFILE: EnterpriseProfile = {
  id: "investor",
  name: "财务投资人 / 基金",
 ...
 */
export const ENTERPRISE_PROFILES: readonly EnterpriseProfile[] = [
  GENERIC_PROFILE,
  FLEET_PROFILE,
  OPERATOR_PROFILE,
  PARK_PROFILE,
...
export const PROFILE_IDS: readonly string[] = ENTERPRISE_PROFILES.map((p) => p.id);

/** 取某个画像；未知 id 诚实回落到「通用」，绝不裸抛（对齐 Scout / 地区包 / 引擎「永不裸抛」范式）。 */
export function getEnterpriseProfile(id: string | null | undefined): EnterpriseProfile {
  return ENTERPRISE_PROFILES.find((p) => p.id === id) ?? GENERIC_PROFILE;
}

/** 供 UI 渲染下拉/切换按钮的精简选项列表。 */
export function listProfileOptions(): { id: string; name: string; summary: string; headline: string }[] {
  return ENTERPRISE_PROFILES.map((p) => ({
    id: p.id,
    name: p.name,
    summary: p.summary,
...
/**
 * 把 R5 的「地区包分层」再垫一层「企业画像预设」，交给 `resolveProjectParams` / `runProjectModel` / `computeTornado`。
 * 这是把 R7 画像接进 R4 工作台与 R2 引擎的唯一接缝：
 *   region/policy（地区默认，来自 R5）垫底 → 画像 `presetValues` → 用户本次显式改动 依次覆盖在上（§14 #7 + §6 优先级）。
 *
 * 关键保证：
 *   - **通用画像 / 无预设 → 与 `buildProjectLayers` 逐字等价**（不挂 source，user 层 values 只含用户改动），
 *     因此不选画像时既有黄金基线绝不受影响；
...
export function buildProfileLayers(
  ...
  if (profile.id === DEFAULT_PROFILE_ID || Object.keys(profile.presetValues).length === 0) {
    return buildProjectLayers(regionId, userValues, now);
  }
...
export function validateProfileCatalog(): string[] {
  ...
  if (ENTERPRISE_PROFILES[0]?.id !== DEFAULT_PROFILE_ID) {
