 ...
 *
 * 客户端只负责：把 R6.1 已算好的**确定性报告** + 可选追问 POST 到受登录门禁的 `/api/workbench/explain`，
...
import type { DecisionReport } from "@app/kernel/lib/decision-report";
...
export function DecisionExplainPanel({ report }: { report: DecisionReport }) {
  ...
  async function run() {
    ...
    setExplanation(null);
    setMeta(null);
    const res = await mutateJson("/api/workbench/explain", "POST", {
      report,
