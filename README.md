...
- [ ] Phase 7 后续 / Phase 13：案例 & 证据 CRUD 的 **HTTP 写路由 + 后台 UI**（现 M5 数据层 + M2 鉴权原语均已就绪，用 `requireRole(STAFF_ROLES)` 保护写端点即可）；行业关联收尾；把 §11 grade 并入可信度公式的后续待决（属公式形态变更，须走 SCORING §5 升版）；更细粒度权限矩阵（按资源/动作）随 Phase 8/12/13 需要再补；随后 Phase 8 方案系统（方案须走真实多角色流水线产出、禁伪造，购买闭环阻塞于 ROADMAP #5 支付）、AI Agent、GitHub Scout…（按 ROADMAP 推进）
