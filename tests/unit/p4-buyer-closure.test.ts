 ...
 *   ④ `ownsSolutionSource` 属主核验：staff 恒放行；无来源指针放行；有 project/scenario 且 ownerId
 *      吻合放行；不吻合或非自有拒绝；DB 异常保守拒绝（绝不因核验失败而放行越权写入）。
 *
 * 集成（真 Neon）留待 P6 测试网阶段一并补：
 *   - `/api/leads` 端到端 + 单实例频控；
 *   - `/api/workbench/solution` 属主 & entitlement 关时 403；
 *   - `findSolutionsBySource` `restrictToCreatorId` 过滤在 JSONB + creatorId 双约束下的实际命中集合。
 */
...
function makeReq(url = "http://localhost:3000/api/workbench/solution", init?: RequestInit): Request {
  return new Request(url, init);
}

describe("P4 · requireUserWrite（登录即可、任意角色）", () => {
  it("CSRF 跨站 → 403（无 Origin 头视为非浏览器，放行到下一环）", async () => {
    const req = makeReq("http://localhost:3000/api/workbench/solution", {
...
describe("P4 · ownsSolutionSource（属主核验：登录 + 属主 端点语义的原语）", () => {
  it("staff（REVIEWER/ADMIN）→ 恒放行，不触库", async () => {
    const r = await ownsSolutionSource({ projectId: PROJ_ID }, STAFF);
    expect(r).toEqual({ owned: true });
    expect(prismaProjectFindUnique).not.toHaveBeenCalled();
  });

  it("无来源指针 / 脏 id → 放行（没有可越权的东西；主链由 verifySolutionSource 处理）", async () => {
    expect(await ownsSolutionSource(null, BUYER)).toEqual({ owned: true });
    expect(await ownsSolutionSource({}, BUYER)).toEqual({ owned: true });
    expect(await ownsSolutionSource({ projectId: "not-a-cuid" }, BUYER)).toEqual({ owned: true });
  });

  it("projectId 存在且 owner 吻合 → 放行", async () => {
    prismaProjectFindUnique.mockResolvedValue({ ownerId: "u_owner" });
    const r = await ownsSolutionSource({ projectId: PROJ_ID }, BUYER);
  ...
  it("scenarioId → 通过其所属项目的 ownerId 判定（本函数只查一次 projectScenario）", async () => {
    ...
    const r = await ownsSolutionSource({ scenarioId: SCEN_ID }, BUYER);
    expect(r.owned).toBe(true);
  ...
  it("scenarioId 属于别人的项目 → 拒绝", async () => {
    ...
    const r = await ownsSolutionSource({ scenarioId: SCEN_ID }, BUYER); // BUYER = u_owner ≠ u_other
    expect(r.owned).toBe(false);
