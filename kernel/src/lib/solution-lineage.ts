 ...
 *   - **稳健的来源判据**：仅当某条财务的 `assumptions.solutionCalcRef` 是以 `SOLUTION_ORIGIN_PREFIX`
 *     （"sandbox-solution@"）起头的字符串时才判定为沙盘来源——该串由 `solution-draft.ts` 的
 *     `solutionDraftCalcRef()` 唯一产出，是人工后台录入方案不会自带的确凿指纹（非启发式猜电价/猜负号）。
 *
 * 边界（勿越权）：**client-safe 纯函数**（与 `solution-draft.ts`/`decision-view.ts`/`decision-report.ts` 同层），
...
export const SOLUTION_LINEAGE_VERSION = "1.0.0";

/**
 * 沙盘来源指纹前缀。`solution-draft.ts` 的 `solutionDraftCalcRef()` 产出 `sandbox-solution@<semver>`，
 * 落到每条财务的 `assumptions.solutionCalcRef`——本层据此确凿判定来源。两模块共享此契约，
 * 由单测「真实草案 → 本层识别」用例钉住防漂移（宪法第 8/16 条单一真源）。
 */
export const SOLUTION_ORIGIN_PREFIX = "sandbox-solution@";
...
function lineageRefOf(financial: LineageFinancialLike): string | null {
  const a = financial.assumptions;
  if (!isRecord(a)) return null;
  const ref = a.solutionCalcRef;
  return typeof ref === "string" && ref.startsWith(SOLUTION_ORIGIN_PREFIX) ? ref : null;
}
 ...
 * 纯函数、确定性；不做任何重算或权限判断。
 */
export function isModelSourcedSolution(financials: readonly LineageFinancialLike[]): boolean {
  return financials.some((f) => lineageRefOf(f) !== null);
}
 ...
 */
export function describeSolutionLineage(
  financials: readonly LineageFinancialLike[],
): SolutionLineage | null {
  if (!Array.isArray(financials)) return null;
  const hit = financials.find((f) => lineageRefOf(f) !== null);
  if (!hit) return null;
  const ref = lineageRefOf(hit) as string;
