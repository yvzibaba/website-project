 ...
 *
 * 职责：把 R8.1 的**纯映射桥**（`solution-draft.ts` 产出的 `SolutionDraft`，在浏览器里由引擎
...
import { attachSolutionSource, type SolutionSourceInput } from "@app/kernel/lib/solution-source";
import { verifySolutionSource } from "@app/kernel/server/solution-source-server";
import {
  createSolution,
 ...
 */
export const SOLUTION_STORE_VERSION = "1.2.0";
 ...
 */
export const SolutionPersistSchema = z.object({
  caseId: CuidSchema,
   ...
   * 两 id 均可选；落库前经 `verifySolutionSource` 查库确有其行才并入财务 assumptions，否则诚实丢弃（绝不虚构关联）。
   */
  sandboxSource: z
    .object({
...
export type SolutionPersistInput = z.infer<typeof SolutionPersistSchema>;

/* ─────────────────────────── 结果类型（与 api-guard.MutationLike 结构兼容，可直喂 mutationResponse） ─────────────────────────── */

export interface SolutionPersistResult {
 ...
 */
export async function persistSolutionDraft(
  input: unknown,
  actor?: string,
  opts?: { creatorId?: string },
): Promise<SolutionPersistResult> {
  const parsed = SolutionPersistSchema.safeParse(input);
  if (!parsed.success) {
  ...
  if (d.sandboxSource) {
    const { ok, ref, note } = await verifySolutionSource(d.sandboxSource as SolutionSourceInput);
    if (ok && ref) {
      financialInputs = attachSolutionSource(financialInputs, ref) as typeof financialInputs;
    } else if (note) {
