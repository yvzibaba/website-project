...
import { upgradeSolutionFinancialProvenance } from "@app/kernel/server/provenance-store";
import { isModelSourcedSolution } from "@app/kernel/lib/solution-lineage";
import { evaluateSolutionProvenance, type FinancialLike } from "@app/kernel/lib/solution-provenance";
 ...
 *   `evaluateSolutionProvenance` 从 0/N 翻转为「可追溯」，且★全程不破坏 R8.3 来源识别 / R8.4 可复算脊柱。
 *
 ...
 *   - 生产地区编目现全为诚实 ASSUMPTION（见 `region-facts`），故这些 FACT 由测试注入，
...
if (!HAS_DB) {
  console.warn("[region-facts] DATABASE_URL not set — integration tests will be skipped.");
}
...
function buildDraftWithRegionSources(layers: Omit<ResolveLayers, "derived">) {
  ...
  const calc: CalcResult = runProjectModel(layers);
  ...
  return { calc, draft: buildSolutionDraft({ calc, vm, regionName: "山西", price: "15800.00", currency: "CNY" }) };
}
...
describeDb("region-facts · 地区真实来源经引擎链→落库→R8.5 升级为可追溯 FACT（Neon）", () => {
  beforeAll(async () => {
    for (let i = 0; i < 4; i++) {
      try {
  ...
  it("①合法来源：解析认 FACT → 草案带 sourceUrl → 落库列+JSONB → R8.5 升级翻转可追溯，且脊柱全程不破", async () => {
    ...
    expect(isModelSourcedSolution(like)).toBe(true); // ★R8.3 沙盘指纹没被新增键冲掉

    const before = await evaluateSolutionProvenance([
      {
        calcRef: row!.calcRef,
    ...
    const pAfter = await evaluateSolutionProvenance(afterLike);
    ...
    expect(isModelSourcedSolution(afterLike)).toBe(true); // ★来源识别仍在（solutionCalcRef 被合并保留）
  });
