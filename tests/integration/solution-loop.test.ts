 ...
 *   - **详情页诚实声明有数可依**：发布后 `describeSolutionLineage(公开详情财务)` 应识别为沙盘来源（供 §16/§20 声明）。
 *
...
if (!HAS_DB) {
  console.warn("[solution-loop] DATABASE_URL not set — integration tests will be skipped.");
}
...
function buildRealDraft(price: string | undefined) {
  ...
  const draft = buildSolutionDraft({ calc, vm, regionName: "山西", price, currency: "CNY" });
...
describeDb("solution-loop · 沙盘来源方案端到端商业闭环（Neon Postgres）", () => {
  beforeAll(async () => {
    for (let i = 0; i < 4; i++) {
      try {
  ...
  it("主链：DRAFT(不可见)→发布(过 publishGuard)→公开可见+沙盘溯源→下单(服务端金额)→确认到账→解锁翻转", async () => {
    ...
    expect(post.data.financials.some((f) => f.calcRef === "model@1.4.0")).toBe(true);
    const lineage = describeSolutionLineage(post.data.financials);
