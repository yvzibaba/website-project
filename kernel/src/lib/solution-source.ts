 ...
 *   - **写侧 `attachSolutionSource`**：对每条财务的 `assumptions` 做**不可变浅合并**（`{...existing, sandboxSource}`），
 *     ★绝不覆盖 `solutionCalcRef`/`roiRatio`/`engineVersions` 等既有溯源键——否则 R8.3 识别 / R8.4 复算会失效。
 *     指针对象缺失（用户从未保存情景 → 无 id 可挂）时**原样返回、绝不虚构**（§20 诚实）。
 *   - **读侧 `readSolutionSourceFromFinancials`**：与 lineage 同构，取**首条**带合法 `sandboxSource` 的财务，
 *     逐字段原值读出、非串/脏值诚实降 `null`，无来源 → `null`（不猜）。既供方案详情页正向展示，也供服务端反查回读。
 *
 * 边界（勿越权）：**client-safe 纯函数**（与 `solution-lineage.ts`/`solution-draft.ts` 同层），
 *   零 DB、零网络、零时钟、零随机、零 `import`（仅 `import type`），入参取结构最小面、不与 Prisma 类型耦合。
 *   「情景 / 项目 id 是否真实存在」的校验属服务端职责（见 `src/server/solution-source-server.ts`），
 *   本层只保证「形状合法 + 不破坏既有脊柱」，绝不冒充已验存在性。本仓刻意不 import "server-only"。
 */

/** 来源关联指针口径版本（字段名 / 形状变化须升版记因，宪法第 13 条）。 */
export const SOLUTION_SOURCE_VERSION = "1.0.0";

/** V1 唯一的来源类型：方案派生自一个「已保存的沙盘情景」。留判别位以备未来其它来源，不硬编码字符串散落。 */
export const SOLUTION_SOURCE_KIND = "sandbox-scenario" as const;

/** 指针落在 `SolutionFinancial.assumptions` 里的键名（写侧 / 读侧 / 服务端反查共享，防拼写漂移，§16）。 */
export const SOLUTION_SOURCE_FIELD = "sandboxSource" as const;

/** 归一后的来源关联指针（只含可确证字段；缺省即整字段省略而非塞空串/塞 null）。 */
export interface SolutionSourceRef {
  kind: typeof SOLUTION_SOURCE_KIND;
...
export interface SolutionSourceInput {
 ...
 */
export function normalizeSolutionSource(input: SolutionSourceInput | null | undefined): SolutionSourceRef | null {
  ...
  const ref: SolutionSourceRef = { kind: SOLUTION_SOURCE_KIND, sourceVersion: SOLUTION_SOURCE_VERSION };
...
function readSourceField(assumptions: unknown): Record<string, unknown> | null {
  ...
  const raw = assumptions[SOLUTION_SOURCE_FIELD];
 ...
 */
export function attachSolutionSource<T extends SourceFinancialLike>(
  financials: readonly T[],
  src: SolutionSourceInput | null | undefined,
): T[] {
  const list = Array.isArray(financials) ? financials : [];
  const ref = normalizeSolutionSource(src);
  if (!ref) return [...list];
  return list.map((f) => {
    const base = isRecord(f.assumptions) ? (f.assumptions as Record<string, unknown>) : {};
    const next: T = { ...f, assumptions: { ...base, [SOLUTION_SOURCE_FIELD]: ref } };
    return next;
  });
}
 ...
 */
export function readSolutionSourceFromFinancials(
  financials: readonly SourceFinancialLike[],
): SolutionSourceRef | null {
 ...
 */
export function describeSolutionSource(ref: SolutionSourceRef | null | undefined): string | null {
  if (!ref) return null;
