...
1. **总控 §11 的证据"等级"轴（S/A/B/C/D）尚未建模**。§11 按**来源权威性**分级（S=政府/法规/原始论文/审计；A=权威机构/年报/专业库；B=行业媒体/报告；C=普通二手；D=AI 推断），这与 §2 用的证据**类型**轴（FACT/ASSUMPTION/…）是**两个正交维度**。当前 prisma `Evidence` 模型**没有 grade 字段**，v1 可信度公式也**未使用等级**。→ **开放决策（M2+）**：是否给 `Evidence` 加 `grade` 枚举并把 §11 分级并入可信度公式（例如 sf 由 sourceUrl 有无升级为按 S/A/B/C/D 取值）。在拍板前，本文档不谎称已覆盖 §11。
