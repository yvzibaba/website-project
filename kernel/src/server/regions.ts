 ...
 *
 * 命名与键：只使用 R1.2 `PROJECT_PARAMS` 里**已注册的键**（如 `region.elecPrice`、`policy.feedInTariff`），
...
import { SHANXI_REGION_SOURCES, SHANXI_POLICY_SOURCES } from "@app/kernel/server/region-facts";

/**
 * 地区参数包目录版本（增删地区 / 改默认口径须升版并记原因，宪法第 13 条）。
 * 1.1.0（R8.7）：山西 region/policy 层挂上来自 `region-facts` 的**逐值结构化溯源** `sources`
 *   （数值一字未改，现全为诚实 ASSUMPTION 占位；核实到权威原文后仅改编目即自动升 FACT 并贯通下游）。
 */
export const REGIONS_VERSION = "1.2.0"; // 1.2.0（V1.1 批次1.2）：山西包 region.demandCharge 44→0（2030 前集中式充换电免需量电费条款直传，52号文 S 级已核实；44 名义价转 B 对照组覆写值）；1.1.0：见 R5/R8.7 史。

/** 溯源引用（供报告标注「这组地区默认是按哪版给的」，第 7/16 条）。 */
export function regionCalcRef(): string {
  return `regions@${REGIONS_VERSION}`;
}
...
/** 通用包：不覆写任何地区/政策值，`resolveProjectParams` 落回 R1.2 全局默认（与 R2/R4 黄金基线逐字一致）。 */
...
export const REGION_PACKS: readonly RegionPack[] = [NATIONAL_PACK, SHANXI_PACK];

/** 默认地区标识（不选地区时 = 全国通用 = R1.2 全局默认，保证与既有黄金基线逐字一致）。 */
export const DEFAULT_REGION_ID = NATIONAL_PACK.id;

/** 合法地区 id 集合（供 UI 与测试遍历）。 */
export const REGION_IDS: readonly string[] = REGION_PACKS.map((p) => p.id);

/** 取某个地区包；未知 id 诚实回落到「全国通用」，绝不裸抛（对齐 Scout/引擎「永不裸抛」范式）。 */
export function getRegionPack(id: string | null | undefined): RegionPack {
  return REGION_PACKS.find((p) => p.id === id) ?? NATIONAL_PACK;
}

/** 供 UI 渲染下拉/切换按钮的精简选项列表。 */
export function listRegionOptions(): { id: string; name: string; summary: string }[] {
  return REGION_PACKS.map((p) => ({ id: p.id, name: p.name, summary: p.summary }));
}

/**
 * 组装「选了这个地区 + 用户覆写」后喂给 `resolveProjectParams` / `runProjectModel` / `computeTornado` 的分层。
