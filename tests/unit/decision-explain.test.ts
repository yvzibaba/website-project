...
import {
  ...
  EXPLAIN_TIER,
  EXPLAIN_AGENT,
  decisionReportInputSchema,
  explanationSchema,
  buildExplainPrompt,
  explainDecisionScenario,
  type DecisionReportInput,
} from "@app/kernel/server/decision-explain";
import {
  StubProvider,
...
const OK_REPORT: DecisionReportInput = {
  reportVersion: "1.0.0",
  ok: true,
...
const FAIL_REPORT: DecisionReportInput = {
  reportVersion: "1.0.0",
  ok: false,
...
describe("decision-explain constants", () => {
...
describe("decisionReportInputSchema", () => {
  it("accepts a well-formed deterministic report", () => {
    const r = decisionReportInputSchema.safeParse(OK_REPORT);
    expect(r.success).toBe(true);
  });

  it("rejects reports with no sections or an unknown section kind", () => {
    expect(decisionReportInputSchema.safeParse({ ...OK_REPORT, sections: [] }).success).toBe(false);
    expect(
      decisionReportInputSchema.safeParse({
        ...OK_REPORT,
...
describe("explainDecisionScenario（编排契约 · 离线 StubProvider）", () => {
  it("honestly refuses to explain an incomplete report (no provider call)", async () => {
    const rec = createMemoryRecorder();
    const res = await explainDecisionScenario(FAIL_REPORT, {}, { provider: BOOM_PROVIDER, recorder: rec });
  ...
  it("returns a validated explanation and records cost on success", async () => {
    const rec = createMemoryRecorder();
    const res = await explainDecisionScenario(
      OK_REPORT,
      {},
  ...
  it("normalizes a provider failure to error without leaking the raw throw as a crash", async () => {
    const res = await explainDecisionScenario(OK_REPORT, {}, { provider: BOOM_PROVIDER });
    expect(res.status).toBe("error");
