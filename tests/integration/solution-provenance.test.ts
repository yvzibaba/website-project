 ...
 *   沙盘结果 → DRAFT（R8.2）→ 人工发布 → 公开详情页财务读回 → `evaluateSolutionProvenance`。
 *
...
if (!HAS_DB) {
  console.warn("[solution-provenance] DATABASE_URL not set — integration tests will be skipped.");
}
...
describeDb("solution-provenance · 沙盘来源方案溯源审计真 Neon 往返", () => {
  beforeAll(async () => {
    for (let i = 0; i < 4; i++) {
      try {
  ...
  it("发布后读回财务：真实列舍入下仍可复算(allReproducible) + 可追溯诚实为 0/N", async () => {
    ...
    const p = evaluateSolutionProvenance(fins);
  ...
  it("篡改落库 roiPct → 再审计揪出复算异常（点名 roiPct，证明是真取证非永真检查）", async () => {
    ...
    const p = evaluateSolutionProvenance(re.data.financials as unknown as FinancialLike[]);
