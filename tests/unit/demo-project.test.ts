...
import { MODEL_VERSION, runProjectModelBaseline } from "@app/kernel/server/project-model";
import {
  DEMO_MODEL_VERSION,
  DEMO_OPERATING_DAYS,
...
} from "@app/kernel/server/demo-project";
...
describe("demo-project · 车队能耗折算（纯映射公式手算）", () => {
...
describe("demo-project · demoUserValues：只垫'用户真动过'的字段", () => {
...
describe("demo-project · V1.1 批次1.1 场站规模缩放链路（P0#1 修复：桩数随车队/功率/窗口变）", () => {
...
describe("demo-project · classifyParameterOrigin 四类真值表（不改引擎枚举）", () => {
...
describe("demo-project · §4 命脉：改一个核心参数 → 网关/经济/图表/报告全链变", () => {
...
describe("demo-project · headline 来源分类在真实情景下正确", () => {
...
describe("demo-project · 全曝光滑杆区间内网关值永不被引擎裁剪（联动可预测）", () => {
...
describe("demo-project · 零churn：默认态经济结果与引擎基线逐字相等", () => {
  it("computeDemoScenario(默认, 无改动) 的 CalcResult 深等于 runProjectModelBaseline()", () => {
    const c = demoBaseline();
    expect(c.calc).toEqual(runProjectModelBaseline());
...
describe("demo-project · 零迁移持久化（塞进 paramLayers 的 demo 块）", () => {
