 ...
 *     原值，正文里的每个金额串等于 `decision-view` 的格式化器输出。
 ...
 *
 * 边界（本轮刻意保守，勿越权）：
 *   - **零 DB、零网络、零时钟、零随机**，纯函数、client-safe（与 `decision-view.ts`/`decision-report.ts` 同层）。
...
import type { CalcResult, CalcResultOk } from "@app/kernel/server/project-model";
import { factInputs } from "@app/kernel/server/parameter-engine";
import { formatMoney, type NamedValue, type DecisionViewModel } from "@app/kernel/lib/decision-view";
import type { EnterpriseProfile } from "@app/kernel/server/profiles";
import { PROFILES_VERSION } from "@app/kernel/server/profiles";
 ...
 */
export const SOLUTION_VERSION = "1.1.1"; // 1.1.1（V1.1 P3 黑话清洗）：草案正文/未知项/财务注记/发布阻塞的买家可见文案人话化（占位假设→示例参数·未经核实、去 §16/§20/E1–E8/ASSUMPTION 枚举直印/caseId 外键等黑话）；结构、字段、计算口径零改动。

/**
 * 溯源引用：把草案钉到「方案生成口径版本」（与各区段引擎 calcRef 并列，供发布后审计）。
 *
 * ★ 命名说明：函数名取 `solutionDraftCalcRef`（= 本模块 `solution-draft.ts` 的口径引用），
 *   与 `financeCalcRef`/`techCalcRef`/`provenanceCalcRef` 同构；**不可**叫 `solutionCalcRef`——
 *   那会与本模块内承载该值的局部量 / 落库字段名 `solutionCalcRef` 同名（P-1 改名时实测踩中 TS2448）。
 *   注意：函数名与 `assumptions.solutionCalcRef` 这个**落库键名**是两件事，后者是历史数据契约，不可改。
 */
export function solutionDraftCalcRef(): string {
  return `sandbox-solution@${SOLUTION_VERSION}`;
}
...
export interface SolutionDraftInput {
  ...
  calc: CalcResult;
  /** 与之配套的视图模型（`buildDecisionViewModel(calc,...)` 产物）——正文金额/百分比串取自此（单一真源）。 */
...
export interface SolutionDraftOk {
  ok: true;
  draftVersion: string;
...
export interface SolutionDraftErr {
  ok: false;
  draftVersion: string;
  solutionCalcRef: string;
...
export type SolutionDraft = SolutionDraftOk | SolutionDraftErr;
...
function card(vm: DecisionViewModel, key: string): string {
 ...
 */
export function buildSolutionDraft(input: SolutionDraftInput): SolutionDraft {
  const { calc, vm, regionName, projectName, scenarioName, profile, price } = input;
  const solutionCalcRef = solutionDraftCalcRef();
  ...
  const profileVersion = profile ? `profiles@${PROFILES_VERSION}` : "通用（无画像裁剪）";
