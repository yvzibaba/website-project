 ...
 * 是纯粹的数据 → 视图投影，因此可像 `scoring.ts` 等其余内核模块一样用黄金样本单测「焊死」
 ...
 *     沙盘链路本身纯函数、client-safe，见 project-model.ts 依赖分析）。
 */

import type { CapexBreakdown, CalcResult, CalcResultOk, OpexBreakdownY1, RevenueBreakdownY1 } from "@app/kernel/server/project-model";
import type { TechFirstYearResult } from "@app/kernel/server/tech";
import type { TornadoResult, TornadoRow } from "@app/kernel/server/sensitivity";
...
/** 年（4.2 → 「4.20 年」；null → 「从不回本」，呼应 finance 的诚实语义）。 */
 ...
 */
export function buildDecisionViewModel(input: ViewModelInput): DecisionViewModel {
