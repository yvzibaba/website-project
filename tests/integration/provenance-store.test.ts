...
} from "@app/kernel/server/provenance-store";
 ...
 *   - **可追溯翻转**：再跑 `evaluateSolutionProvenance` → `traceableCount:1`（原 0）、`fullyTraceable:false`（部分），
 *     buyerSummary 出现「已升级为 FACT 且带真实来源链接」。
 *   - **★不破坏复算**：升级只改溯源、绝不碰 Decimal 列/源值 → 同一行 `verifyReproducibility` 仍 pass、`allReproducible:true`。
 *   - **★不破坏来源识别**：`assumptions.solutionCalcRef` 被合并保留 → `isModelSourcedSolution` 升级后仍为 true（R8.3 徽章不丢）。
...
if (!HAS_DB) {
  console.warn("[provenance-store] DATABASE_URL not set — integration tests will be skipped.");
}
...
async function persistPublishedDraft(tag: string): Promise<string> {
  ...
  const draft = buildSolutionDraft({ calc, vm, regionName: "山西", price: "19800.00", currency: "CNY" });
...
describeDb("provenance-store · 沙盘来源财务 ASSUMPTION→FACT 升级写路径真 Neon 往返", () => {
  beforeAll(async () => {
    for (let i = 0; i < 4; i++) {
      try {
  ...
  it("合法来源升级一条财务：FACT 落库 + version 自增 + 可追溯翻转，且复算/来源识别仍成立（不破坏脊柱）", async () => {
    ...
    const pBefore = evaluateSolutionProvenance(before);
    ...
    const pAfter = evaluateSolutionProvenance(after);
    ...
    expect(isModelSourcedSolution(after)).toBe(true);
  });
  ...
  it("无来源链接 → blocked 且库中该行逐字段未变（§20 拒不粉饰占位假设）", async () => {
    ...
    const pAfter = evaluateSolutionProvenance(await readFinancials(solutionId));
    expect(pAfter.traceability.traceableCount).toBe(0);
  });

  it("STORE_VERSION 语义化 + 编排函数对空/非法意图稳健（不裸抛）", async () => {
    expect(PROVENANCE_STORE_VERSION).toMatch(/^\d+\.\d+\.\d+$/);
