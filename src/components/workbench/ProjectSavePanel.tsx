/**
 * 沙盘「项目保存 / 版本 / 回滚」面板（中途重构 R6.3 · §14 优先级第 2 项「项目模型」落到 /workbench）。
 *
 * 职责边界（诚实）：本组件**只搬运输入、绝不本地算数**——把工作台当前的参数分层（含 `now` 与政策日期窗）
 * 原样 POST 给受登录 + owner 门禁的服务端，由 `project-store` **重跑引擎**落库快照与 Decimal 汇总列（§4 命脉）。
...
export function ProjectSavePanel({
  ...
  async function loadLinkedSolutions(sid: string) {
    ...
    const res = await mutateJson(`/api/workbench/source/solutions?scenarioId=${encodeURIComponent(sid)}`, "GET");
    setLinkedBusy(false);
    if (res.status === 401) {
  ...
  async function refreshVersions(sid: string) {
    const res = await mutateJson(`/api/workbench/scenarios/${sid}/versions`, "GET");
    if (res.ok) setVersions(((res.data as { versions?: VersionRow[] })?.versions) ?? []);
  }

  async function refreshProject(pid: string) {
    const res = await mutateJson(`/api/workbench/projects/${pid}`, "GET");
    if (res.ok) {
  ...
  async function updateScenario() {
    ...
    setBusy(true);
    setNotice(null);
    const res = await mutateJson(`/api/workbench/scenarios/${scenarioId}`, "PUT", { layers });
    setBusy(false);
  ...
  async function saveVersion() {
    ...
    setBusy(true);
    setNotice(null);
    const res = await mutateJson(`/api/workbench/scenarios/${scenarioId}/versions`, "POST", {
  ...
  async function restoreTo(versionId: string, seq: number) {
    ...
    setBusy(true);
    setNotice(null);
    const res = await mutateJson(`/api/workbench/scenarios/${scenarioId}/restore`, "POST", { versionId });
    setBusy(false);
