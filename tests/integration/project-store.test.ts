...
if (!HAS_DB) console.warn("[project-store] DATABASE_URL not set — integration tests will be skipped.");

const num = (d: unknown): number | null => (d == null ? null : Number(d));

describeDb("project-store 项目/情景/版本持久层（Neon Postgres）", () => {
  ...
  it("createProject：建项目 + 基线情景，现算落库 calcStatus ok + Decimal 汇总列对齐 R2 golden", async () => {
    ...
    expect(sc.calcRef).toBe("model@1.5.0");
  ...
  it("updateScenarioLayers：越界单价被裁剪到边界仍算得出（clamp 不致失败，非缺参）", async () => {
    ...
    // （缺经济键的失败路径无法经 resolveProjectParams 造出——R1.2 模板全键皆有占位默认，故此处验裁剪而非缺键。）
    const upd = await updateScenarioLayers(
      created.scenarioId,
  ...
  it("createProject：regionId 非规范 Region 行（沙盘地区包代码）→ 不报 P2003、诚实置空（§17 E2E 回归）", async () => {
    // "shanxi" 是 regions 的内存包代码，不是 Region 表行——旧实现塞进外键会 500。
    const res = await createProject({
 ...
 * ─────────────────────────────────────────────────────────────────────────────────────────────── */
describeDb("project-store 内核升级冻结策略（真连 Neon）", () => {
  ...
  it("模型升级后重存（calcRef model@1.0.0 → 1.5.0）→ 旧 ok 结果自动冻结，数字逐字段不变、storage=none、可恢复", async () => {
    ...
    // 新引擎（model@1.5.0）重算 → 必须先冻结旧结果再覆写。
    const res = await updateScenarioLayers(sc.id, {}, { actor });
    expect(res.ok).toBe(true);
    if (!res.ok) return;
    ...
    expect(after.calcRef).toBe("model@1.5.0");
  ...
  it("同 calcRef 但 engineVersions 指纹变化（历史行缺 storage 键）→ 同样触发冻结（旧行受保护）", async () => {
    ...
    // 模拟「1.5.0 首日行」：calcRef 不变，但 engineVersions 没有 storage 键（本批部署前落库）。
    ...
    const versions = await listScenarioVersions(sc.id);
    expect(versions).toHaveLength(1);
    expect(versions[0].frozen.engineVersions.model).toBe("1.5.0");
