 ...
 * 沙盘中台图表（中途重构 R4 可视化）。
 *
 * 纯展示件（client）：只吃 `src/lib/decision-view.ts` 已算好并格式化过的扁平数据，
 * **不在这里做任何经济/技术计算**（第 7 条程序算、第 16 条单一真源）——所有数字来自引擎，
 * 图表绑定模型（§8），改参数→引擎重算→数据变→图变，全在 `ProjectWorkbench` 里串起来。
 *
...
import type { CashFlowPoint, NamedValue, TornadoBar } from "@app/kernel/lib/decision-view";
