 ...
 *   - 每次写入前先 `runProjectModel(layers)` 现算，落 `calcResult` 快照 + 从快照派生的少量精确汇总列；
 *   - 读回 / 回滚时**重新跑引擎**（`updateScenarioLayers` / `restoreScenarioFromVersion`），
 *     使「改参数 → 引擎重算 → 经济结果变」这条链在持久层同样成立，而不是把旧数字搬来搬去。
 *
 * 货币精确（第 15 条唯一动用 Decimal 处）：引擎内部估算是 Number（`finance`），但**落库**的
 ...
 *    本层**不含鉴权**——「谁能建/改哪个项目」由 R6.3 接上的路由层（`/api/workbench/**`）用
 *    `requireSameOriginActor`（登录）+ `project-service` 的 owner-or-staff 判定把关，本层只认调用方已鉴权。
 */
...
const log = logger.child({ module: "server/project-store" });
...
function computeScenarioData(layers: StoredParamLayers) {
  ...
  const calc = runProjectModel(engineLayers);
  const numeric = resolveProjectParams(engineLayers).numeric;
  return {
...
export async function createProject(input: CreateProjectInput): Promise<CreateProjectResult> {
  ...
  if (regionId) {
    ...
    if (!regionRow) {
      log.info("workbench regionId not a canonical Region row; leaving Project.regionId null", { regionId });
      regionId = null;
    }
  }
  ...
  try {
    ...
    log.info("workbench project created", { projectId: project.id, calcStatus: data.calcStatus });
