...
import {
  ...
  computeTornado,
  deriveRiskFlags,
} from "@app/kernel/server/sensitivity";
import { runProjectModelBaseline, runProjectModel, type CalcResultOk } from "@app/kernel/server/project-model";

describe("sensitivity · 版本", () => {
...
describe("deriveRiskFlags · 高风险标记（§16）", () => {
  it("健康基线：恒 needsProfessionalReview + 含'简化'与'示例参数'两条基础理由（V1.1 P3：占位假设→示例参数措辞随动）", () => {
    const { needsProfessionalReview, flags } = deriveRiskFlags(runProjectModelBaseline());
  ...
  it("亏损情景（充电单价过低）→ 追加'NPV<0 不可行'标记", () => {
    const loss = runProjectModel({ user: { values: { "project.chargingPrice": 0.4 } } });
    const { flags } = deriveRiskFlags(loss);
    expect(flags.some((f) => f.includes("NPV<0"))).toBe(true);
  });

  it("模型失败 → 直接标记不可用须人工核查", () => {
    const broken = runProjectModel({ user: { values: { "project.pvCapacity": NaN } } });
...
describe("阶段1 · spread 解锁合理性（SENSITIVITY_VERSION 1.3.0）", () => {
  ...
  it("低/中/高 spread：储能价值与 NPV 严格单调增、IRR 单调增、折现回收期严格单调减", () => {
    const at = (values: Record<string, number>): CalcResultOk => {
      const r = runProjectModel({ user: { values } });
  ...
  it("spread=0 → 储能价值诚实归零（套利腿关闭，消纳腿 S1 年度互斥恒 0）", () => {
    const r = runProjectModel({ user: { values: { "region.peakValleySpread": 0 } } });
