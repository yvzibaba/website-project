 ...
 * `WorkbenchCharts` + `DecisionReportPanel` + `ProjectSavePanel`），本文件**不做任何经济计算、不新增 API 路由、零 schema 迁移**。
 *
 * 高级用户点「展开完整参数工作台」即切到既有 `ProjectWorkbench`（40 参数全量），R8.8a 不因此扩展开发范围。
 */
...
import { DEFAULT_REGION_ID, getRegionPack, listRegionOptions } from "@app/kernel/server/regions";
import type { Tone } from "@app/kernel/lib/decision-view";
import type { MetricCard } from "@app/kernel/lib/decision-view";
import { formatMoney } from "@app/kernel/lib/decision-view";
...
export function DemoProjectPanel({
  onOpenFull,
  initial,
}: {
  onOpenFull?: () => void;
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[minmax(0,340px)_1fr]">
        ...
        <Card className="h-fit">
          ...
          <CardContent className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-2">
              ...
              <RegionClauseFacts regionId={state.regionId} />
            </div>
        ...
        <div className="flex flex-col gap-6">
          ...
          {showSave && vm.ok ? (
            <ProjectSavePanel layers={savedLayers} regionId={state.regionId} regionName={`${pack.name}（示范项目）`} />
          ) : null}
        </div>
      </div>
    </div>
