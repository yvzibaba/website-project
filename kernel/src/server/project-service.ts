 ...
 *
 * 为什么单独一层（在 R3 `project-store` 之上）：
 *   - `project-store` 刻意「不含鉴权」（见其头注），把「谁能建/改哪个项目」交给调用方；本层就是那个调用方，
 *     把这条安全底线收敛到**一处**（宪法第 16 条单一真源 / §28 权限测试重点），供 4 条 `/api/workbench/**` 路由共用，
 *     杜绝每个端点各写一遍 owner 判断而漂移。
 *   - 命脉（§4）：本层**不自己算数**，一律转交 `project-store`，后者在写入/回滚时 `runProjectModel` 现算落库快照 +
...
} from "@app/kernel/server/project-store";

const log = logger.child({ module: "server/project-service" });
...
export type ProjectActionResult<T extends Record<string, unknown> = Record<string, never>> =
  | ({ status: "ok" } & T)
...
/** 与 ok 泛型无关的失败臂——可直接并入任意 `ProjectActionResult<T>`（供鉴权不通过时提前 return）。 */
type Denied =
  | { status: "invalid"; fieldErrors: Record<string, string[]> }
  | { status: "not_found"; error?: string }
  | { status: "forbidden"; error?: string }
...
async function accessByScenarioId(scenarioId: string, user: SessionUser): Promise<Access> {
  ...
  if (!canAccessProject(sc.project?.ownerId, user)) {
    log.warn("workbench scenario access denied", { scenarioId, userId: user.id, role: user.role });
...
export async function createProjectForUser(
...
): Promise<ProjectActionResult<{ projectId: string; scenarioId: string }>> {
  ...
  if (res.ok) {
    log.info("workbench project created via api", { projectId: res.projectId, userId: ctx.user.id });
...
export async function updateScenario(
  ...
  body: unknown,
  ctx: { user: SessionUser },
): Promise<ProjectActionResult<{ calcStatus: string; version: number }>> {
...
export async function saveScenarioVersion(
  ...
  body: unknown,
  ctx: { user: SessionUser },
): Promise<ProjectActionResult<{ versionId: string; seq: number }>> {
  const access = await accessByScenarioId(scenarioId, ctx.user);
  if (!access.ok) return access.result;
...
export async function readProject(
  projectId: string,
  ctx: { user: SessionUser },
): Promise<ProjectActionResult<{ project: unknown }>> {
...
/** 列出当前会话用户自己的沙盘项目（「我的项目」列表页 / GET /api/workbench/projects 共用）。 */
export async function listProjects(
  ctx: { user: SessionUser },
): Promise<ProjectActionResult<{ projects: ProjectListItem[] }>> {
  try {
    ...
    log.error("listProjects failed", { err });
...
export async function copyProject(
  ...
  body: unknown,
  ctx: { user: SessionUser },
): Promise<ProjectActionResult<{ projectId: string; scenarioId: string; name: string }>> {
  const access = await accessByProjectId(projectId, ctx.user);
  if (!access.ok) return access.result;
  ...
  if (res.ok) {
    log.info("workbench project copied", { from: projectId, to: res.projectId, userId: ctx.user.id });
...
export async function readScenarioVersions(
  scenarioId: string,
  ctx: { user: SessionUser },
): Promise<ProjectActionResult<{ versions: unknown }>> {
  const access = await accessByScenarioId(scenarioId, ctx.user);
  if (!access.ok) return access.result;
