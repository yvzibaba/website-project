/**
 * R8.8a — /workbench 页面的**双档外壳**（客户端 · 极薄，只做模式切换与项目载入，不含任何计算逻辑）。
 *
 * 落地《R8.8 最小实施方案》§8「普通用户简单易懂、高级用户可展开更多」的裁决：
 *   - 默认「示范项目模型」档：露出 ~10 个核心参数（`DemoProjectPanel`），改参数即调用**既有**沙盘引擎整链重算；
 *   - 「完整参数工作台」档：原样渲染既有 `ProjectWorkbench`（40 参数全量，R1–R7 主链，一字未改）。
 * 两档切换**只在客户端切组件**，不改路由（`/workbench` 仍单一 `○` 静态路由、零新增）、不碰后端、不碰存储。
 *
...
import { parseBaselineLayers, type ProjectRestore } from "@app/kernel/lib/project-restore";
import { DemoProjectPanel } from "./DemoProjectPanel";
...
export function WorkbenchShell() {
  ...
  useEffect(() => {
    ...
    let alive = true;
    mutateJson(`/api/workbench/projects/${encodeURIComponent(projectParam)}`, "GET")
      .then((res) => {
        if (!alive) return;
        if (res.status === 401) {
          router.replace(`/login?callbackUrl=${encodeURIComponent(`/workbench?project=${projectParam}`)}`);
          return;
        }
        if (!res.ok) {
