...
import { runProjectModel, type CalcResult, type CalcResultOk } from "@app/kernel/server/project-model";
import { projectCalcToColumns, STORE_VERSION } from "@app/kernel/server/project-store";

const base = runProjectModel();
if (!base.ok) {
  // 基线必须成功（R2 已锁），否则本文件所有黄金值失效——尽早失败并指名。
  throw new Error("基线 runProjectModel 未成功，测试前置不成立");
}
const BASELINE = base;

describe("project-store · projectCalcToColumns（CalcResult → Decimal 汇总列，纯映射）", () => {
  ...
  it("负 NPV / 负回收期值 → 定点串如实带符号（不夹逼到 0）", () => {
    const neg = runProjectModel({ user: { values: { "project.chargingPrice": 0.5 } } });
