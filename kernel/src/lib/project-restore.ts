 ...
 *   - 完整工作台项目（layers 带 `wb` 块，ProjectWorkbench 保存时附加的地区/画像 id）→
 *     还原地区 id、画像 id 与用户覆写；user 层里「与画像预设同值」的键被扣除，
 *     让画像预设还原为「画像默认」归属，而非伪装成用户已改（引擎本就把二者都报成 origin=user，
 *     这层区分在应用侧用数据算，绝不臆测——与 profiles.isProfileDefault 同一纪律）。
 *
 * 纯函数、零 React / 零网络依赖（可离线单测）；未知 id 不在此处裁决，交给面板诚实回落默认
 * （getRegionPack / getEnterpriseProfile 对未知 id 恒回落，绝不裸抛）。
 * 引擎只读 region/policy/user/now（见 project-store.toEngineLayers），`demo`/`wb` 是刻意加性
 * 透传块，不参与任何计算，也不会被引擎改写。
 */
import { deserializeDemoState, type DemoHeadlineState, type DemoTouched } from "@app/kernel/server/demo-project";
