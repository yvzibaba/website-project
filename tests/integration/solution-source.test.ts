...
import { createProject } from "@app/kernel/server/project-store";
import { findSolutionsBySource, verifySolutionSource } from "@app/kernel/server/solution-source-server";
import { SOLUTION_SOURCE_FIELD, readSolutionSourceFromFinancials } from "@app/kernel/lib/solution-source";
 ...
 *   反查 findSolutionsBySource（scenario / project 两向）找回该方案。
 *
 * 关键实证（§12 可追溯 / §16 不破坏脊柱 / §20 诚实丢假指针）：
 *   - ★关联不伤脊柱：挂了指针后，落库行 `solutionCalcRef` 指纹仍在（isModelSourcedSolution 仍 true）、
...
if (!HAS_DB) {
  console.warn("[solution-source] DATABASE_URL not set — integration tests will be skipped.");
}
...
function buildRealDraft() {
  ...
  const vm = buildDecisionViewModel({ calc, tech: tech && tech.ok ? tech.firstYear : null, tornado, discountRate });
  return buildSolutionDraft({ calc, vm, regionName: "山西", price: "12800.00", currency: "CNY" });
}
...
async function persistWithTag(tag: string, sandboxSource?: { scenarioId?: string; projectId?: string }) {
  ...
  const res = await persistSolutionDraft(
    {
      caseId: c.id,
      title: draft.title,
...
describeDb("solution-source · 沙盘情景↔方案反查关联端到端（Neon Postgres）", () => {
  beforeAll(async () => {
    for (let i = 0; i < 4; i++) {
      try {
  ...
  it("①主链：保存情景→导出带指针→验存落库→反查两向命中 + ★关联不伤 R8.3/R8.4 脊柱", async () => {
    const { projectId, scenarioId } = await newProject("happy");
    ...
    const ref = readSolutionSourceFromFinancials([{ assumptions: fin!.assumptions }]);
    expect(ref).not.toBeNull();
    expect(ref!.scenarioId).toBe(scenarioId);
    expect(ref!.projectId).toBe(projectId);

    // ★不破坏既有脊柱：沙盘指纹仍在 → isModelSourcedSolution true；Decimal/源值仍可复算。
    ...
    expect(isModelSourcedSolution(like)).toBe(true);
    ...
    // 反查：按 scenarioId 命中。
    const byScen = await findSolutionsBySource({ scenarioId });
    ...
    const byProj = await findSolutionsBySource({ projectId });
    expect(byProj.status).toBe("ok");
    if (byProj.status === "ok") {
      expect(byProj.items.some((x) => x.id === solutionId)).toBe(true);
    }
    // 反查：两 id 都给，仍命中且回读指针吻合。
    const both = await findSolutionsBySource({ scenarioId, projectId });
  ...
  it("②诚实丢假指针：指向不存在情景 → 导出仍成功但行内无 sandboxSource（+ warnings）", async () => {
    ...
    expect(a[SOLUTION_SOURCE_FIELD]).toBeUndefined(); // 绝不落一条通往虚空 id 的假关联
    // 但沙盘指纹仍在（草案自身写的）。
    expect(typeof a.solutionCalcRef).toBe("string");

    // 反查一个不存在的情景 → 空。
    const r = await findSolutionsBySource({ scenarioId: fakeScenario });
    expect(r.status).toBe("ok");
    if (r.status === "ok") expect(r.count).toBe(0);
  });

  it("③验存函数直连：真情景 ok / 假情景 拒 / 情景项目不一致 拒", async () => {
    const { projectId, scenarioId } = await newProject("verify");
    const good = await verifySolutionSource({ scenarioId, projectId });
    expect(good.ok).toBe(true);
    expect(good.ref?.scenarioId).toBe(scenarioId);

    const missing = await verifySolutionSource({ scenarioId: "c" + "g".repeat(24) });
    expect(missing.ok).toBe(false);
    expect(missing.ref).toBeNull();
    expect(missing.note).toContain("不存在");

    // 情景真、项目假（张冠李戴）→ 拒，防错挂。
    const mismatch = await verifySolutionSource({ scenarioId, projectId: "c" + "h".repeat(24) });
    expect(mismatch.ok).toBe(false);
    expect(mismatch.ref).toBeNull();

    // 无 id → 静默 null。
    const none = await verifySolutionSource({});
  ...
  it("④未保存情景导出（不带指针）→ 行内无 sandboxSource、反查它为空", async () => {
    ...
    expect((fin!.assumptions as Record<string, unknown>)[SOLUTION_SOURCE_FIELD]).toBeUndefined();
    // 无任何 (scenario/project) 反查会命中它。
    const { scenarioId, projectId } = { scenarioId: "c" + "q".repeat(24), projectId: "c" + "w".repeat(24) };
    const r = await findSolutionsBySource({ scenarioId, projectId });
    expect(r.status).toBe("ok");
