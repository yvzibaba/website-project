...
- [ ] Phase 6 里程碑 2+ / Phase 7 后续：角色/权限中间件（SECURITY §4，**有意延后到 Phase 13**——`/admin` 尚不存在，此刻建门禁违反宪法第 2/4 条 MVP 优先；`UserRole` 枚举已在库就绪）；随后案例 CRUD / 证据管理（含总控 §11 证据等级轴 S/A/B/C/D 建模的开放决策——是否给 Evidence 加 grade 并入可信度公式）、行业关联、Phase 8+ 方案系统、AI Agent、GitHub Scout…（按 ROADMAP 推进）
