...
import { saveScenarioVersion, readScenarioVersions } from "@app/kernel/server/project-service";

/**
 * /api/workbench/scenarios/[id]/versions — 版本时间线（R6.3）。
 *
...
export async function POST(request: Request, { params }: Ctx): Promise<NextResponse> {
  ...
  const result = await saveScenarioVersion(id, parsed.data, { user: gate.user });
...
export async function GET(request: Request, { params }: Ctx): Promise<NextResponse> {
  ...
  if ("response" in gate) return gate.response;
  const { id } = await params;
  const result = await readScenarioVersions(id, { user: gate.user });
  return mutationResponse(result);
}
