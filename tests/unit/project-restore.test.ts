...
import { parseBaselineLayers } from "@app/kernel/lib/project-restore";
import { defaultDemoState } from "@app/kernel/server/demo-project";
import { PROFILE_IDS } from "@app/kernel/server/profiles";
...
describe("parseBaselineLayers（沙盘 ?project= 还原解析）", () => {
  ...
  it("wb 块 → 工作台还原：与画像预设同值的键被扣除（归属诚实），异值保留，字符串覆写丢弃", () => {
    // fleet 预设含 project.trucksPerDay=150、finance.discountRate=10（见 profiles）。
    const layers = {
  ...
  it("未知 wb.profileId → 全部 user 值按用户覆写还原（预设查找回落通用空预设，绝不裸抛）", () => {
    ...
    expect(PROFILE_IDS).not.toContain("not-a-profile");
  });
});
