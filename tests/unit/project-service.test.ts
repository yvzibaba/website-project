...
// 离线单测：顶掉 prisma / logger / authz / project-store，专注验证「鉴权真值表 + 编排契约」，绝不触库、绝不打真模型。
...
vi.mock("@app/kernel/server/project-store", () => ({
  createProject: vi.fn(),
...
import {
  ...
  readProject,
  readScenarioVersions,
  PROJECTS_VERSION,
} from "@app/kernel/server/project-service";
import * as store from "@app/kernel/server/project-store";
...
describe("project-service · 常量与输入契约", () => {
  it("PROJECTS_VERSION 是语义化版本串", () => {
    expect(PROJECTS_VERSION).toMatch(/^\d+\.\d+\.\d+$/);
  });
...
describe("project-service · canAccessProject（§28 权限真值表 / 纯函数）", () => {
  it("owner 本人可访问自己的项目", () => {
...
describe("project-service · createProjectForUser（ownerId 强制服务端会话）", () => {
  it("成功：忽略客户端伪造的 ownerId、注入会话 user.id + actor，透传 layers 交 store 现算", async () => {
    ...
    const res = await createProjectForUser(
  ...
  it("空名 → invalid，且不触 store", async () => {
    const res = await createProjectForUser({ name: "  " }, { user: USER });
  ...
  it("store 报错 → 归一 error", async () => {
    ...
    const res = await createProjectForUser({ name: "x" }, { user: USER });
    expect(res).toEqual({ status: "error", error: "数据库错误(P2002)" });
  });
});

describe("project-service · 情景级操作先过 owner-or-staff（越权在动库前拦）", () => {
  it("update：非 owner USER → forbidden，store 绝不被调用", async () => {
    ...
    const res = await updateScenario("s1", { layers: { user: { values: {} } } }, { user: INTRUDER });
  ...
  it("update：情景不存在 → not_found", async () => {
    ...
    const res = await updateScenario("ghost", { layers: {} }, { user: USER });
  ...
  it("update：owner 通过 → 调 store 重算并回 ok", async () => {
    ...
    const res = await updateScenario("s1", { layers: { user: { values: { "project.chargingPrice": 1.5 } } } }, { user: USER });
  ...
  it("update：STAFF 非 owner 亦可（后台治理）", async () => {
    ...
    const res = await updateScenario("s1", { layers: {} }, { user: REVIEWER });
  ...
  it("saveVersion：owner 通过 → store 收到 savedBy actor", async () => {
    ...
    const res = await saveScenarioVersion("s1", { label: "调电价后" }, { user: USER });
  ...
  it("saveVersion：非 owner USER → forbidden，store 不被调用", async () => {
    findScenario.mockResolvedValue({ id: "s1", projectId: "p1", project: { ownerId: "u-owner" } });
    const res = await saveScenarioVersion("s1", {}, { user: INTRUDER });
    expect(res.status).toBe("forbidden");
  ...
  it("restore：缺 versionId → invalid，store 不被调用", async () => {
    findScenario.mockResolvedValue({ id: "s1", projectId: "p1", project: { ownerId: "u-owner" } });
    const res = await restoreScenarioVersion("s1", {}, { user: USER });
    expect(res.status).toBe("invalid");
  ...
  it("restore：store 判版本不属此情景(forbidden) → 透传 forbidden", async () => {
    ...
    const res = await restoreScenarioVersion("s1", { versionId: "vx" }, { user: USER });
    expect(res).toEqual({ status: "forbidden", error: "该版本不属于此情景" });
  });
});

describe("project-service · 读取（精简视图，不外泄快照/Decimal）", () => {
  it("readProject：owner 通过 → 映射 Decimal→number、Date→ISO、无 calcResult", async () => {
    ...
    const res = await readProject("p1", { user: USER });
    expect(res.status).toBe("ok");
  ...
  it("readProject：非 owner USER → forbidden，store 不被调用", async () => {
    findProject.mockResolvedValue({ id: "p1", ownerId: "u-owner" });
    const res = await readProject("p1", { user: INTRUDER });
    expect(res.status).toBe("forbidden");
  ...
  it("readVersions：STAFF 可读他人项目版本时间线（含冻结摘要：model/storage 版本 + 冻结指标，缺 storage 键映射 none）", async () => {
    ...
    const res = await readScenarioVersions("s1", { user: ADMIN });
    expect(res.status).toBe("ok");
