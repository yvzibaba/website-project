 ...
 *
 * 边界（勿越权）：
 *   - **client-safe 纯函数**（与 `solution-draft.ts`/`solution-lineage.ts`/`decision-view.ts` 同层）：
 ...
 *     两个**存量**之间的一致性，不调用 `runProjectModel`、不换算 NPV。这是审计，不是再算一遍。
...
export const SOLUTION_PROVENANCE_VERSION = "1.0.0";

/** 溯源引用：把一次审计钉到「本口径版本」，供页面与审计日志展示（与 lineage 的 solutionCalcRef 并列不冲突）。 */
export function provenanceCalcRef(): string {
  return `sandbox-solution-provenance@${SOLUTION_PROVENANCE_VERSION}`;
}
...
export interface SolutionProvenance {
  auditVersion: string;
 ...
 */
export function evaluateSolutionProvenance(
  ...
  return {
    auditVersion: SOLUTION_PROVENANCE_VERSION,
