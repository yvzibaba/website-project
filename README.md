...
- [ ] Phase 6 里程碑 2+ / Phase 7 后续：角色/权限中间件（SECURITY §4，**有意延后到 Phase 13**——`/admin` 尚不存在，此刻建门禁违反宪法第 2/4 条 MVP 优先；`UserRole` 枚举已在库就绪）；随后案例 CRUD / 把 §11 grade 并入可信度公式的后续待决（属公式形态变更，须走 SCORING §5 升版）、行业关联、Phase 8+ 方案系统、AI Agent、GitHub Scout…（按 ROADMAP 推进）
