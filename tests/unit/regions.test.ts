...
describe("regions — 地区/政策参数包目录（中途重构 R5，§6）", () => {
  it("常量与版本守护", () => {
    expect(REGIONS_VERSION).toMatch(/^\d+\.\d+\.\d+$/);
    expect(regionCalcRef()).toBe(`regions@${REGIONS_VERSION}`);
    expect(REGION_IDS).toContain(DEFAULT_REGION_ID);
    // 「全国通用」恒为首个且即默认包，保证不选地区时 = R1.2 全局默认（黄金基线不变）。
    expect(REGION_PACKS[0].id).toBe(DEFAULT_REGION_ID);
    expect(REGION_IDS).toEqual(REGION_PACKS.map((p) => p.id));
  });
  ...
  it("listRegionOptions：精简选项与目录一一对应", () => {
    ...
    expect(opts).toHaveLength(REGION_PACKS.length);
    for (const o of opts) {
  ...
  it("buildProjectLayers：组合 region 垫底 + policy + user 在上 + 注入 now", () => {
    const layers = buildProjectLayers("shanxi", { "project.trucksPerDay": 80 }, NOW_ACTIVE);
    expect(layers.region?.values["region.elecPrice"]).toBe(0.55);
    expect(layers.policy).toBe(REGION_PACKS[1].policy);
    expect(layers.user?.values).toEqual({ "project.trucksPerDay": 80 });
    expect(layers.now).toEqual(NOW_ACTIVE);
    // 通用包不覆写任何地区/政策值。
    const nat = buildProjectLayers("national");
    ...
    expect(buildProjectLayers("shanxi").now).toBeInstanceOf(Date);
  });

  it("§6 地区默认：选山西后电价/光照来自 region 层、补贴/上网价来自在效政策层", () => {
    const r = resolveProjectParams(buildProjectLayers("shanxi", {}, NOW_ACTIVE));
    ...
    // 通用包：全部落回 R1.2 全局默认。
    const nat = resolveProjectParams(buildProjectLayers("national", {}, NOW_ACTIVE));
  ...
  it("★§6 命脉：过期政策绝不作现行默认（碳价加计落回全局默认 + 留痕）", () => {
    ...
    const now = resolveProjectParams(buildProjectLayers("shanxi", {}, NOW_ACTIVE));
    ...
    const back = resolveProjectParams(buildProjectLayers("shanxi", {}, NOW_EXPIRED_ERA));
  ...
  it("§6 用户区间内覆写：用户覆写盖过地区默认，且被地区限幅（透传到 user 层的 bounds）裁剪", () => {
    ...
    const r = resolveProjectParams(buildProjectLayers("shanxi", { "region.elecPrice": 0.4 }, NOW_ACTIVE));
    ...
    expect(buildProjectLayers("shanxi").user?.bounds?.["region.elecPrice"]).toEqual({ min: 0.35, max: 0.9 });
    // 越上界 → 裁到 0.9；越下界 → 裁到 0.35（诚实标 clamped，不静默放过）。
    const hi = resolveProjectParams(buildProjectLayers("shanxi", { "region.elecPrice": 5 }, NOW_ACTIVE));
    expect(hi.params["region.elecPrice"].value).toBe(0.9);
    expect(hi.params["region.elecPrice"].clamped).toBe(true);
    const lo = resolveProjectParams(buildProjectLayers("shanxi", { "region.elecPrice": 0.1 }, NOW_ACTIVE));
  ...
  it("★§4/§6 命脉：换地区→默认参数变→经济结果即时重算（非只改页面数字）", () => {
    const nat = runProjectModel(buildProjectLayers("national", {}, NOW_ACTIVE));
    const sx = runProjectModel(buildProjectLayers("shanxi", {}, NOW_ACTIVE));
    ...
    expect(computeTornado({ layers: buildProjectLayers("shanxi", {}, NOW_ACTIVE) }).baseValue).toBeCloseTo(
      sx.metrics.npv,
      2,
    );
  ...
  it("确定性：同 (地区, 覆写, now) 两次解析深相等", () => {
    const a = resolveProjectParams(buildProjectLayers("shanxi", { "project.pvCapacity": 800 }, NOW_ACTIVE));
    const b = resolveProjectParams(buildProjectLayers("shanxi", { "project.pvCapacity": 800 }, NOW_ACTIVE));
