 ...
 */
export const PARAMETER_SPECS: readonly ParameterSpec[] = [
  ...
  num("region.demandCharge", "需量(容量)电价", "region", "advanced", 0, "元/kW·月", {
    ...
    // 免收需量(容量)电费」条款取 0（山西 52 号文 S 级原文已核实，见 region-facts；
    // 全国同条款文号未逐条归档 → 目录默认仍标 ASSUMPTION + 强标注，不伪 FACT）。
    // 不适用免征/免征到期 → B 普通工商业场景：本键覆写 40（全国）/44（山西名义工商业价）。
    source:
      "【占位假设·主情景=免征口径 A】默认 0 系按「2030 年前实行两部制电价的经营性集中式充换电设施免收需量(容量)电费」条款字面直传（山西 52 号文原文已核实，来源钉在 region-facts；全国同条款文号待补归档，故目录默认不标 FACT）。非集中式/免征到期请覆写 40–44（B 对照组）或按报装容量全额计（C 压力测试，Kc=100）。",
    confidence: 50,
  }),
  ...
  // ── R9.0 Step 2 · SVE 储能价值五键（接入 storage-value.ts，全为占位假设/年度代理口径）──
...
export const PROJECT_PARAMS: readonly ParameterSpec[] = ParameterSetSchema.parse(
  PARAMETER_SPECS,
);
 ...
 * 派生值重算 → 交 R2 重算经济 → 图表/报告联动。默认已挂上 `DERIVED`，调用方只传覆写层。
 */
export function resolveProjectParams(layers: Omit<ResolveLayers, "derived"> = {}): ResolveResult {
  return resolveParameters(PROJECT_PARAMS, {
    derived: DERIVED,
    ...layers,
  });
}
...
export function resolveProjectParamsBaseline(): ResolveResult {
  return resolveProjectParams();
}
