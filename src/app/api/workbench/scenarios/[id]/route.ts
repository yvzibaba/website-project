...
import { updateScenario } from "@app/kernel/server/project-service";

/**
 * /api/workbench/scenarios/[id] (PUT) — 改某情景的参数分层 → **服务端重跑引擎**落新快照（version++）（R6.3）。
 *
...
export async function PUT(request: Request, { params }: Ctx): Promise<NextResponse> {
  ...
  const result = await updateScenario(id, parsed.data, { user: actor.user });
