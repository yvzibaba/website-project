 ...
 *   （CSRF 同源 + 必须登录）+ **属主双闸**：① 普通用户查询的情景/项目必须归其所有（`ownsSandboxSource`，
...
export async function GET(request: Request): Promise<NR> {
  ...
  if (!isStaff) {
    const owner = await ownsSandboxSource({ scenarioId, projectId }, guard.user);
    if (!owner.owned) {
      return errorResponse("FORBIDDEN", owner.reason ?? "只能反查属于自己的沙盘情景 / 项目", 403);
    }
  }

  const result = await findSolutionsBySandboxSource(
