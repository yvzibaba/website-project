...
- [ ] Phase 6 里程碑 2+ / Phase 7 里程碑 3+：角色/权限中间件（SECURITY §4，**有意延后到 Phase 13**——`/admin` 尚不存在，此刻建门禁违反宪法第 2/4 条 MVP 优先；`UserRole` 枚举已在库就绪）；评分拆解上详情页（M3：getPublicCaseById 返回 scoreBreakdown + UI + DEMO 种子补 scoreInput 演示）；随后案例 CRUD/证据管理（含总控 §11 等级轴建模开放决策）、Phase 8+ 方案系统、AI Agent、GitHub Scout…（按 ROADMAP 推进）
