...
import { MODEL_VERSION, type CalcResultOk } from "@app/kernel/server/project-model";
...
describe("TASK3 · 默认零 churn 保护（映射层刻意不 bump 经济内核版本）", () => {
  it("默认无改动 → calc 与 runProjectModelBaseline() 深等 + MODEL_VERSION 跟随引擎（不自行 bump）", () => {
