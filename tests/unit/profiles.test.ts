 ...
 *  ② 未知/空 id 诚实回落通用（绝不裸抛）；
 *  ③ ★通用画像 → 分层与 R5 `buildProjectLayers` 逐字等价（不选画像时 R6 黄金基线绝不受影响）；
...
import { buildProjectLayers } from "@app/kernel/server/regions";
...
const SPEC = new Map(PROJECT_PARAMS.map((s) => [s.key, s] as const));

describe("profiles — 企业画像目录（R7 · §14 第 7 项）", () => {
  it("常量与版本守护", () => {
    expect(PROFILES_VERSION).toMatch(/^\d+\.\d+\.\d+$/);
    expect(profileCalcRef()).toBe(`profiles@${PROFILES_VERSION}`);
    expect(PROFILE_IDS).toContain(DEFAULT_PROFILE_ID);
    // 通用画像恒为首个 = 默认，保证不选画像时逐字回落到既有黄金基线。
    expect(ENTERPRISE_PROFILES[0].id).toBe(DEFAULT_PROFILE_ID);
    expect(PROFILE_IDS).toEqual(ENTERPRISE_PROFILES.map((p) => p.id));
  });
  ...
  it("listProfileOptions：精简选项与目录一一对应，字段齐备", () => {
    ...
    expect(opts).toHaveLength(ENTERPRISE_PROFILES.length);
    for (const o of opts) {
  ...
  it("★诚实不变式（第 20 条 / §16）：任何有预设值的画像都标 ASSUMPTION·低置信·带待核实标注", () => {
    for (const p of ENTERPRISE_PROFILES) {
  ...
  it("★通用画像 → 分层与 R5 buildProjectLayers 逐字等价（不选画像=黄金基线不受影响）", () => {
    const a = buildProfileLayers(DEFAULT_PROFILE_ID, "shanxi", { "project.pvCapacity": 999 }, NOW);
    const b = buildProjectLayers("shanxi", { "project.pvCapacity": 999 }, NOW);
    expect(a).toEqual(b);
    // 无用户改动时同样等价。
    expect(buildProfileLayers(DEFAULT_PROFILE_ID, "national", {}, NOW)).toEqual(buildProjectLayers("national", {}, NOW));
  });
  ...
  it("§14#7 优先级：画像预设盖过全局默认（origin=user），用户本次改动又盖过画像预设", () => {
    ...
    const onlyProfile = resolveProjectParams(buildProfileLayers("fleet", "national", {}, NOW));
    ...
    const userWins = resolveProjectParams(buildProfileLayers("fleet", "national", { "project.trucksPerDay": 300 }, NOW));
  ...
  it("§6 地区限幅仍透传到 user 层：选山西后越界电价被裁到该省上下界（画像路径不破坏地区收窄）", () => {
    const hi = resolveProjectParams(buildProfileLayers("fleet", "shanxi", { "region.elecPrice": 5 }, NOW));
  ...
  it("★§4 命脉：换画像 → 引擎重算 → NPV 真变（个性化进经济链，非只改页面文字）", () => {
    const base = runProjectModel(buildProfileLayers(DEFAULT_PROFILE_ID, "national", {}, NOW));
    const investor = runProjectModel(buildProfileLayers("investor", "national", {}, NOW));
    const operator = runProjectModel(buildProfileLayers("operator", "national", {}, NOW));
  ...
  it("确定性：同 (画像, 地区, 覆写, now) 两次解析深相等", () => {
    const a = resolveProjectParams(buildProfileLayers("park", "shanxi", { "project.chargerCount": 9 }, NOW));
    const b = resolveProjectParams(buildProfileLayers("park", "shanxi", { "project.chargerCount": 9 }, NOW));
