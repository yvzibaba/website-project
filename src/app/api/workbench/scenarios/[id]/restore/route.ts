...
import { restoreScenarioVersion } from "@app/kernel/server/project-service";

/**
 * /api/workbench/scenarios/[id]/restore (POST) — 回滚到某历史版本（R6.3）。
 *
...
export async function POST(request: Request, { params }: Ctx): Promise<NextResponse> {
  ...
  const result = await restoreScenarioVersion(id, parsed.data, { user: actor.user });
