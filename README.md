...
- [ ] Phase 6 里程碑 2+ / Phase 7 后续：角色/权限中间件 + `/admin` 门禁与案例/证据 CRUD 的 HTTP 写路由与 UI（SECURITY §4，**有意延后到 Phase 13**——数据层 CRUD 已在 M5 就绪，此刻建无鉴权端点违反宪法第 2/4 条 MVP 优先；`UserRole` 枚举已在库就绪）；随后行业关联收尾、把 §11 grade 并入可信度公式的后续待决（属公式形态变更，须走 SCORING §5 升版）、Phase 8+ 方案系统、AI Agent、GitHub Scout…（按 ROADMAP 推进）
