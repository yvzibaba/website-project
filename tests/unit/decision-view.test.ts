/**
 * 沙盘中台「视图模型」`src/lib/decision-view.ts` 黄金样本（中途重构 R4 可视化）。
 *
 * 覆盖两类断言：
 *   1) 纯投影：手搓 CalcResult/Tech/Tornado fixture，逐项锁死格式化与图表数据形状（含诚实降级：
 *      NaN/null → 「—」、从不回本、多根示警、失败不画脏图）。
 *   2) ★§8「图表绑定模型」实证：把**真实引擎**（runProjectModelBaseline / computeTechModel /
 *      computeTornado）的输出喂进 buildDecisionViewModel，断言呈现数字与模型输出严格一致
...
} from "@app/kernel/lib/decision-view";
import type { CalcResultOk } from "@app/kernel/server/project-model";
import type { TechFirstYearResult } from "@app/kernel/server/tech";
import type { TornadoResult } from "@app/kernel/server/sensitivity";
...
describe("decision-view · 中文格式化纯函数", () => {
...
describe("decision-view · 指标卡着色与诚实", () => {
...
describe("decision-view · buildDecisionViewModel 装配", () => {
  ...
  it("ok 装配全部数据集；tech/tornado 缺省时对应块为 undefined", () => {
    const vm = buildDecisionViewModel({ calc: okCalc() });
  ...
  it("meta：计算期 = 现金流长度 − 1；货币标签来自模型", () => {
    const vm = buildDecisionViewModel({ calc: okCalc(), tech });
...
describe("decision-view · ★§8 图表绑定真实模型输出", () => {
  it("真实 runProjectModelBaseline + computeTechModel + computeTornado → 视图数字与模型严格一致", () => {
    const calc = runProjectModelBaseline();
    expect(calc.ok).toBe(true);
    if (!calc.ok) return;
    const techRes = computeTechModel(resolveProjectParams().numeric);
    const tornado = computeTornado();
    const vm = buildDecisionViewModel({ calc, tech: techRes.ok ? techRes.firstYear : null, tornado, discountRate: 0.08 });
  ...
  it("改参数（充电单价↑）→ 视图 NPV 卡随引擎重算而升（§4 命脉在呈现层的体现）", async () => {
    const { runProjectModel } = await import("@app/kernel/server/project-model");
    const base = runProjectModelBaseline();
    const hi = runProjectModel({ user: { values: { "project.chargingPrice": 1.2 } } });
    expect(base.ok && hi.ok).toBe(true);
    if (!base.ok || !hi.ok) return;

    expect(hi.metrics.npv).toBeGreaterThan(base.metrics.npv);

    const vmBase = buildDecisionViewModel({ calc: base });
    const vmHi = buildDecisionViewModel({ calc: hi });
