...
import { shouldAutoFreezeVersion, frozenVersionSummary, STORE_VERSION } from "@app/kernel/server/project-store";
import { STORAGE_MODEL_VERSION } from "@app/kernel/server/storage-value";
...
describe("project-store 冻结策略 · STORE_VERSION / STORAGE_MODEL_VERSION", () => {
...
describe("project-store 冻结策略 · shouldAutoFreezeVersion（判据表）", () => {
  const base = {
...
describe("project-store 冻结策略 · frozenVersionSummary（原样提取，不重算）", () => {
