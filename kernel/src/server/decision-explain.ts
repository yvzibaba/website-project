 ...
 *
 * 为什么存在：R6.1（v0.43.0）已把引擎输出确定性地叙述成一份中文报告（`buildDecisionReport`），
 ...
 *   - LLM 的**唯一输入**是 R6.1 已经算好的确定性报告（`DecisionReport` 的 JSON），它的每个数字都是程序算出的；
 ...
 *   绝不可打进浏览器 bundle；故解释必须走一条受门禁的 `POST /api/workbench/explain`（登录用户方可消耗真实额度）。
...
const log = logger.child({ module: "server/decision-explain" });
...
/**
 * 客户端回传的「当前沙盘报告」的最小结构校验（与 `@/lib/decision-report` 的 `DecisionReport` 对齐）。
 ...
 */
export const decisionReportInputSchema = z.object({
...
export type DecisionReportInput = z.infer<typeof decisionReportInputSchema>;
...
function flattenReportForPrompt(report: DecisionReportInput): string {
  const lines: string[] = [];
 ...
 */
export async function explainDecisionScenario(
  report: DecisionReportInput,
  ...
  log.info("explained workbench scenario", {
