 ...
 *   无 DATABASE_URL 自动 skip（对齐 project-store 集成测）。
 */

const HAS_DB = Boolean(process.env.DATABASE_URL);
const describeDb = HAS_DB ? describe : describe.skip;
if (!HAS_DB) console.warn("[demo-project] DATABASE_URL not set — integration tests will be skipped.");
...
describeDb("R8.8a 示范项目模型 · 存版→重开一致（Neon Postgres）", () => {
  ...
  it("改车队规模存版 → 服务端重算==客户端整链、demo 块往返保真、汇总列对齐（验收 #4/#5/#6/#9）", async () => {
    ...
    const replayCalc = runProjectModel(replayLayers);
  ...
  it("示范默认态（无改动）存版 → 汇总列 == R1.2 既有黄金基线（端到端零 churn）", async () => {
    ...
    projectIds.push(res.projectId);
    const sc = (await getProjectWithScenarios(res.projectId))!.scenarios[0];
    // 数字焊自 project-store 集成基线（全国通用、无覆写；R9.0 含 Δ_sto）：证明打开示范模型、什么都不改 == 引擎基线。
