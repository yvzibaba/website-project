...
/**
 * /workbench — 「新能源重卡 + 光伏 + 储能 + 充电」产业项目可视化决策沙盘（中途重构 R4）。
 *
 * 这是《项目中途重构总控》把 V1 唯一焦点落成的人可交互页面：左侧改参数、右侧技术 / 经济 /
 * 图表 / 敏感性即时重算联动（§4 命脉）。页面壳为 Server Component，重活全在 client
 * `WorkbenchShell`（纯前端即时重算，无网络往返）。
 *
 * R8.8a：`WorkbenchShell` 提供双档——默认「示范项目模型」（~10 核心参数简化入口）与
...
export const metadata: Metadata = {
  title: "产业项目决策平台",
  description:
    "新能源重卡 + 光伏 + 储能 + 充电一体化项目的可视化决策平台：改参数即重算技术结果、经济评价（NPV/IRR/回收期/ROI）、能量平衡与敏感性，图表与模型实时联动。",
  ...seoMetadata({
    title: "产业项目决策平台",
    description:
      "改参数即重算技术、经济、风险与敏感性的一体化产业项目可视化决策平台（结果需专业人工确认）。",
    path: "/workbench",
  }),
};
