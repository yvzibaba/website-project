 ...
 * 用 R2.4 `runProjectModel` **整链重算**（真正走 §4 命脉：改滑块→技术→现金流→指标全变），
 ...
 *   - 本模块不改模型经济口径，只是**重复调用** `runProjectModel` 观察输出响应，故与 R2.4 共享同一套简化假设与 `needsProfessionalReview`。
 */
import {
  PARAMETER_SPECS,
...
import { runProjectModel, runProjectModelBaseline, type CalcResult } from "@app/kernel/server/project-model";
 ...
 *   · `project.gridCapacity`——未参与功率约束（P1-3），加了会显示"改了无反应"的伪敏感；
 *   · `region.landRent` / `policy.carbonPrice` /
...
const LABELS: Record<string, { label: string; unit?: string }> = Object.fromEntries(
  PARAMETER_SPECS.map((s) => [s.key, { label: s.label, unit: s.unit }]),
);
...
/** 基线数值快照（无覆写，解析一次即缓存）。 */
const BASELINE_NUMERIC: Record<string, number> = resolveProjectParams().numeric;
...
export function computeTornado(
  ...
  const base = layers ? runProjectModel(layers) : runProjectModelBaseline();
  const baseValue = metricValue(base, metric);

  // 基线各参数生效值：取自参数引擎对应分层的数值快照（R1.2 单一真源；有情景则按情景解析）。
  const baseNumeric = layers ? resolveProjectParams(layers).numeric : BASELINE_NUMERIC;
  ...
  for (const p of params) {
    ...
    const lowRes = runProjectModel(withUserOverride(layers, p.key, lowInput));
    const highRes = runProjectModel(withUserOverride(layers, p.key, highInput));
...
export function deriveRiskFlags(res: CalcResult): {
  ...
  const flags: string[] = [
    "本平台为透明简化年度估算（非可研/逐时/财税级），投资与并网决策须经专业人工确认",
