 ...
 *
 * 喂**真实引擎输出**（`runProjectModel` + `resolveProjectParams` + `computeTechModel` + `computeTornado` → `buildDecisionViewModel`）
 * 构造 vm 再产草案，与 `decision-report.test.ts` 同构，端到端焊死「进方案的数 = 模型算的数」。
 */
import { describe, it, expect } from "vitest";
import {
  buildSolutionDraft,
  solutionDraftCalcRef,
  SOLUTION_VERSION,
} from "@app/kernel/lib/solution-draft";
...
function okDraft(calc = runProjectModelBaseline(), overrides: Record<string, unknown> = {}) {
...
describe("solution-draft · 版本与契约（R8.1）", () => {
  it("版本语义化 + calcRef 溯源串", () => {
    expect(SOLUTION_VERSION).toMatch(/^\d+\.\d+\.\d+$/);
    expect(solutionDraftCalcRef()).toBe(`sandbox-solution@${SOLUTION_VERSION}`);
  });
...
describe("solution-draft · §7 单一真源（财务=引擎原值、正文=卡片串，绝不重算）", () => {
  ...
  it("★正文 ROI/回收期/成本/收入分节的数字串取自视图模型卡片（呈现层搬运）", () => {
    ...
    const resolved = resolveProjectParams();
    const tech = calc.ok ? computeTechModel(resolved.numeric) : null;
    const vm = buildDecisionViewModel({ calc, tech: tech && tech.ok ? tech.firstYear : null, tornado: calc.ok ? computeTornado() : null });
    const roiCard = vm.cards?.find((c) => c.key === "roi")?.value ?? "";
    const paybackCard = vm.cards?.find((c) => c.key === "payback")?.value ?? "";
    const d = buildSolutionDraft({ calc, vm, regionName: "山西" });
    expect(d.ok).toBe(true);
    if (!d.ok) return;
...
describe("solution-draft · §16/§20 诚实边界", () => {
  it("★失败情景只回诚实错误、绝不产出可售正文/财务", () => {
    ...
    const d = buildSolutionDraft({ calc: badCalc, vm, regionName: "山西" });
  ...
  it("★负 NPV 被点名进风险叙述与发布阻塞项（不粉饰）", () => {
    const calcDown = runProjectModel({ user: { values: { "project.chargingPrice": 0.5 } } });
  ...
  it("★未定价 → 阻塞项点名；传入价格 → 解除该项且落 draft.price", () => {
    ...
    const dPriced = okDraft(runProjectModelBaseline(), { price: "1999.00" });
...
describe("solution-draft · R7 画像裁剪进入方案", () => {
  it("带画像 → 适用企业用画像名/摘要 + 摘要与溯源带画像版本", () => {
    const fleet = getEnterpriseProfile("fleet");
    const d = okDraft(runProjectModelBaseline(), { profile: fleet });
    expect(d.ok).toBe(true);
    if (!d.ok) return;
...
describe("solution-draft · 确定性（同输入两次深相等，可复算）", () => {
  it("同 calc + 同上下文两次生成完全相等", () => {
    const calc = runProjectModelBaseline();
    const a = okDraft(calc);
    const b = okDraft(runProjectModelBaseline());
    expect(a).toEqual(b);
  });
});
...
function draftFromCalc(calc: ReturnType<typeof runProjectModel>) {
  const resolved = resolveProjectParams();
  const tech = calc.ok ? computeTechModel(resolved.numeric) : null;
  const vm = buildDecisionViewModel({
    calc,
  ...
  return buildSolutionDraft({ calc, vm, regionName: "山西", projectName: "晋中站", scenarioName: "基准情景" });
}

describe("solution-draft · R8.7 逐输入溯源与 sourceUrl 填充（喂 R8.5 升级写路径）", () => {
  it("★存在可核验 FACT 来源时：行级 sourceUrl 落地 + assumptions.inputProvenance + 诚实措辞，但 evidenceGrade 仍 ASSUMPTION", () => {
    const calc = runProjectModel({
      region: {
  ...
  it("★诚实基线回归：全 ASSUMPTION（factCount=0）→ 草案逐字如旧（无 sourceUrl / 无 inputProvenance 键 / 原文措辞）", () => {
    const calc = runProjectModelBaseline(); // 现已带 inputProvenance，但全为 ASSUMPTION
