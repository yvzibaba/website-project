...
import { explainDecisionScenario, decisionReportInputSchema } from "@app/kernel/server/decision-explain";

/**
 * /api/workbench/explain — 对**当前沙盘情景的确定性报告**追加一段 AI 自然语言解释（中途重构 R6.2）。
 *
 ...
 *
 * 铁律（§7 / 第 16 条）：LLM **只解释不算数**——输入是前端已算好的 `DecisionReport`（其数字全来自引擎），
 *   服务端不重算、不接受客户端传入的「新结论」，只把报告原样喂给编排器解释；成本经 ModelCall 记录器落库。
 *   请求体 `{ report: DecisionReport, question?: string }`；判别联合由 `mutationResponse` 统一翻译。
 */
...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  const reportParse = decisionReportInputSchema.safeParse(body.report);
  ...
  const result = await explainDecisionScenario(reportParse.data, { question }, { recorder });
